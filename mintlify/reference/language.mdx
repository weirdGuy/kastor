...
| `requires_approval` | optional | Subset of `tools` that pauses for a human before execution. |
...
- `tools` is the grant; `requires_approval` only narrows it. Every approval entry must also appear in `tools`, duplicates are errors, and omission from `tools` is the denial. LangGraph emits `HumanInTheLoopMiddleware`, eve emits its authored-tool or connection approval gate, and `claude_agents` emits `always_ask` for the named tools. Richer per-target decision types and predicates are deliberately not part of the portable language.
